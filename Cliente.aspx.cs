...
public partial class Cliente : System.Web.UI.Page
{
    ...
    {
        SqlDataReader lector = sql.consulta("SELECT * FROM Cliente");
        try
        {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    int registro = sql.ejecutar("INSERT INTO Cliente (Run,Nombre,Apellido,Usuario,Telefono,CorreoE,Direccion,comuna_id,Clave,id_Estado) VALUES ('" + txt_rut.Text + "','" + txt_nombre.Text + "','" + txt_apellido.Text + "','" + txt_usuario.Text + "','" + txt_telefono.Text + "','" + txt_correo.Text + "','" + txt_direc.Text + "','" + DropDownList4.SelectedValue + "','" + txt_pass.Text + "','" + DropDownList7.SelectedValue + "')");
                                    if (registro > 0)
                                    {
    ...
    {
        sql.llenacombo(DropDownList7, "select * from Estado", "Estado", "id_Estado");
    }

    protected void llenaGrilla()
    {
        sql.llenaGrid(UsuarioGrid, "SELECT * FROM Cliente");
    }

    protected void UsuarioGrid_SelectedIndexChanged(object sender, EventArgs e)
    {
